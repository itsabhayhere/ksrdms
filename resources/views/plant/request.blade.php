...
@section('content')
    ...
    <div class="fcard margin-fcard-1 clearfix">
        ...
        <div class="upper-controls clearfix">
            <div class="fl">
                <h3>Verify Dairies</h3>
                <div class="light-color f-12">Total: {{count($requests)}}</div>
            </div>
                ...
                <tbody>
                    ...
                    @foreach ($requests as $r)
                        <tr class="dairyReq{{$r->id}}">
                            ...
                            <td>
                                <a href="javascript:void(0);" onclick="completeRequest({{ $r->dairyId}}, 'Accept');" class="btn btn-default btn-sm"> Verify </a>
                                <a href="javascript:void(0);" onclick="completeRequest({{ $r->dairyId}}, 'Decline');" class="btn btn-danger btn-sm"> Unverify </a>
                            </td>
                        </tr>
                    @endforeach
    ...
    function completeRequest(id, action){
        $.confirm({
            // icon: 'fa fa-trash-o',
			title: (action=='Accept')?'Verify dairiy?':'Remove Dairy?',
			content: 'Are you sure to '+action+' this request?',
			type: (action=='Accept')?'green':'red',
			typeAnimated: true,
			buttons: {
				accept: {
					text: (action=='Accept')?'Verify':'Remove',
